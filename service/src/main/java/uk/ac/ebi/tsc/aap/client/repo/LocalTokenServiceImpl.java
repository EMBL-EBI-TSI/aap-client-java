...
import uk.ac.ebi.tsc.aap.client.repo.LocalTokenService;
...
public class LocalTokenServiceImpl implements LocalTokenService {
    ...
    public LocalTokenServiceImpl(
            @Value("${aap.auth.url}") String domainsApiUrl,
    ...
    public ResponseEntity<String> getAAPTokenResponse(String username, String password) {
        HttpEntity<String> request = new HttpEntity<String>(createHttpHeaders(username,password));
